...
export default function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="w-full bg-[#F7F5F0] py-14 sm:py-20 overflow-hidden border-b border-[#1A1814]/10">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, idx) => (
            <div
              key={idx}
              className="rounded-[28px] bg-white border border-[#1A1814]/10 p-5 sm:p-6 shadow-xs hover:shadow-md hover:border-[#1E4334]/30 transition-all duration-300 flex flex-col justify-between text-left"
            >
              <div>
                {/* Step Thumbnail */}
                <div className="relative h-36 rounded-none overflow-hidden bg-stone-100 mb-4 border border-[#1A1814]/10">
                  <img
                    src={step.img}
                    alt={step.title}
                    className="w-full h-full object-cover"
                  />
                  <div className="absolute top-2.5 left-2.5 px-2.5 py-0.5 rounded-full bg-[#1E4334] text-white text-[11px] font-serif font-medium shadow-xs">
                    Step {step.num}
                  </div>
                </div>

                <span className="text-[11px] font-semibold text-[#1E4334] px-2.5 py-0.5 rounded-full bg-[#1E4334]/8 border border-[#1E4334]/12 inline-block mb-2">
                  {step.tag}
                </span>
