...
export default function CueBundleSection({ onOpenRoleModal }: CueBundleSectionProps) {
  ...
  return (
    <section id="solutions" className="w-full bg-[#F7F5F0] py-16 sm:py-24 overflow-hidden border-b border-[#1A1814]/10">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-12">
          {cards.map((card, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`rounded-[28px] p-7 bg-white border border-[#1E4334]/12 shadow-xs hover:border-[#1E4334]/35 hover:shadow-md transition-all duration-300 text-left flex flex-col justify-between ${card.span}`}
              >
                <div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 p-6 sm:p-8 rounded-[32px] bg-[#1E4334] text-[#F7F5F0]">
          ...
          <button
            ...
            className="px-8 py-3.5 rounded-full bg-[#C8F028] text-[#1A1814] font-semibold text-sm hover:bg-white transition-all shadow-md shrink-0"
          >
