...
export default function CueBundleSection({ onOpenRoleModal }: CueBundleSectionProps) {
  ...
  return (
    <section id="solutions" className="w-full bg-[#FAF7F2] py-16 sm:py-24 overflow-hidden border-b border-[#1B382B]/10">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="text-left mb-12">
          ...
          <h2 className="font-serif text-3xl sm:text-5xl lg:text-6xl text-[#1F1914] tracking-tight leading-[1.12] mb-3 font-normal">
            Thoughtfully built for Indian elders &amp; families
          </h2>
          <p className="font-serif text-xl sm:text-2xl text-[#1F1914]/75 italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-12">
          {cards.map((card, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`rounded-[28px] p-7 bg-white border border-[#1B382B]/12 shadow-xs hover:border-[#D97706]/40 hover:shadow-md transition-all duration-300 text-left flex flex-col justify-between ${card.span}`}
              >
                <div>
                  <div className="flex items-center justify-between mb-5">
                    <div className="w-11 h-11 rounded-2xl bg-[#1B382B]/8 text-[#1B382B] flex items-center justify-center border border-[#1B382B]/15">
                      <Icon className="w-5 h-5 text-[#1B382B]" />
                    </div>
                    <span className="text-[11px] font-semibold text-[#1B382B] px-3 py-1 rounded-full bg-[#1B382B]/8 border border-[#1B382B]/12">
                      {card.tag}
                    </span>
                  </div>

                  <h3 className="font-serif text-xl sm:text-2xl font-normal text-[#1B382B] tracking-tight mb-2.5">
                    {card.title}
                  </h3>
        ...
        {/* Banner Action Row */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 p-6 sm:p-8 rounded-[32px] bg-[#1B382B] text-[#FAF7F2]">
          ...
          <button
            ...
            className="px-8 py-3.5 rounded-full bg-[#E58A18] hover:bg-[#F59E0B] text-[#1F1914] font-semibold text-sm transition-all shadow-md shrink-0 cursor-pointer"
          >
